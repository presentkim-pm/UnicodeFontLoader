...
final class Main extends PluginBase{
	...
	protected function onEnable() : void{
		$this->fontsDir = Path::join($this->getServer()->getDataPath(), "resource_packs/fonts");
		...
		if(!is_dir($this->fontsDir)){
			...
			foreach([
				...
				$this->seperateGlyph($this->getResourcePath($resourcePath), Path::join($this->fontsDir, $glyphName));
			}
		}
		...
		foreach(scandir($this->fontsDir) as $file){
			if(preg_match("/^glyph_[0-9A-F]{2}\.png$/i", $file) === 0){
				continue;
			}

			$glyphPath = Path::join($this->fontsDir, $file);
			...
			try{
				$this->seperateGlyph(
					$glyphPath,
					Path::join($this->fontsDir, "glyph_$prefixHex")
				);
			}catch(\Exception $e){
		...
		foreach(scandir($this->fontsDir) as $file){
			...
			$glyphDir = Path::join($this->fontsDir, $file);
			if(!is_dir($glyphDir)){
				continue;
			}
		...
		$this->getLogger()->info("Load unicode font glyphs from : $this->fontsDir");
		if($addonPath === null){
